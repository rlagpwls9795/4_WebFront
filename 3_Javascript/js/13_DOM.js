...
document.getElementById("btn1").addEventListener("click", function(){
    ...
    // 요소.firstChild : 요소의 첫 번째 자식 노드 탐색해서 반환
    // 요소.lastChild : 요소의 마지막 자식 노드 탐색해서 반환
    // 요소.childNodes[인덱스] : 요소의 인덱스 번쩨 자식 노드를 탐색해서 반환 

    // 형제 
    // 요소.previousSibling : 요소의 이전 형제 노드를 반환
    // 요소.nextSibling : 요소의 다음 형제 노드를 반환

    // #test의 자식 노드를 모두 얻어 와서 확인
    ...
    // #li1의 부모의 배경색 orange로 변경 (노드 탐색 활용 방법)
    li1.parentNode.style.backgroundColor="orange";

    // #li1의 부모 노드 마지막 부분에 "ABCD"라는 text node 추가
    ...
    console.log(test.childNodes[5].previousSibling); //[4]
    // #test의 자식 노드 중 5번 인덱스의 다음 형제 노드
    console.log(test.childNodes[5].nextSibling); //[6]
    
    // 노드 탐색 구문 연속 작성
    // #test의 자식 노드 중 5번 인덱스의 이전 형제 노드의 이전 형제 노드
    console.log(test.childNodes[5].previousSibling.previousSibling); //[3]
...
document.getElementById("btn2").addEventListener("click", function(){
    ...
    // children : 자식 요소만 모두 선택 (HTMLCollection 형태)
    ...
    list[2].previousElementSibling.addEventListener("click", function(){
        ...
        // 이벤트가 발생한 요소의 다음 다음 형제 요소의 배경색 red로 변경
...
btn4.addEventListener("click", function(){
    ...
    // 요소A.append(요소B) => 요소A의 마지막 자식으로 추가
    div.append(input);
