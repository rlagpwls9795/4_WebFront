...
function fnTest1(){
    ...
    //id가 div1인 요소를 얻어 와 상수형 변수 div1에 저장
    const div1 = document.getElementById("div1");

    //접근한 요소의 배경색을 얻어 와 변수에 저장
...
function fnTest3(){
    ...
    //문서 내에 존재하는 모든 li 요소를 얻어 와 배열 형식으로 반환
    const arr = document.getElementsByTagName("li");

    for(let i=0;i<arr.length;i++){
        const num=arr[i].innerText;
        arr[i].style.backgroundColor="rgb(100,"+num+",200)";
    }
...
function fnTest5(){
    ...
    //name 속성값이 hobby인 요소를 모두 얻어 와 배열 형식으로 반환
    ...
    for(let i=0;i<hobby.length;i++){
        ...
        //.checked 속성 : 해당 요소가 체크되어 있다면 true / 아니면 false
        //(radio button, checkbox 전용 속성)
        if(hobby[i].checked) {
...
function fnTest6(){
    ...
    //id 속성 값이 "cssTest"인 요소를 1개 선택 (여러 요소가 선택된 경우 첫 번째 요소만 선택)
...
/* 키보드 입력 과정
    키 누름 -> keydow 이벤트 -> input 태그에 입력 -> keypress -> (손을 떼면) keyup
*/
...
function readValue(){
    ...
    if(input.value.trim().length>0){ //문자열.trim() : 문자열 좌우공백(중간공백X) 제거
