//innerHTML로 읽어오기 (요소 내부 내용(태그+속성+내용)을 얻어 와 문자열로 반환)
function getInnerHTML1(){

    // HTML 문서 내에서 id 속성값이 test1인 요소를 얻어 와
    // 상수형 변수 t1에 저장
...
function add(){
    ...
    // 1) id 속성값이 area1인 요소를 얻어와 변수 area1에 저장
...
//innerText로 읽어오기 (내용만 얻어 옴)
function getInnerText1(){
    //console.log 자동완성 ==> clg (JS ES6 code snippets 필요)
    console.log(t2.innerText);
    console.log(t2.innerHTML);
}

//innerText로 변경하기 (태그, 속성이 해석되지 않음)
function setInnerText1(){
    t2.innerText = "<b class='red'>변경된 내용입니다.</b>";
}
...
function fnConfirm(){
    ...
    if(confirm("배경색을 분홍색으로 바꾸시겠습니까?")){
        ...
        document.getElementById("confirmTest").style.backgroundColor="grey";
    }
}

//prompt
function fnPrompt(){
    const input = prompt("변경할 버튼명을 입력하세요"); //입력한 값 or null
