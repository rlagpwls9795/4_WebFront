...
document.getElementById("btn1").addEventListener("click", function(){
    ...
    //객체 값을 얻어오거나 대입하는 방법
    // 1) 얻어 오기 : 객체명.key     또는 객체명['key']
    // 2) 대입 :      객체명.key=값; 또는 객체명['key']=값;
...
document.getElementById("btn2").addEventListener("click", function(){
    ...
    const stdList = []; // 비어 있는 배열

    // 2. 생성자 함수 호출 : new 생성자함수명(매개변수)
    stdList.push(new Student(1,2,"홍길동"));
    stdList.push(new Student(1,3,"김길동"));
    stdList.push(new Student(2,10,"박길동"));
    stdList.push(new Student(3,5,"최길동"));
...
function Student(grade, ban, name){ 
    // JS 함수의 매개변수는 let, const, var를 적지 않음 (안 적어도 해당 함수의 지역변수로 취급)

    // 생성자 함수에서의 this == new 연산자를 통해 만들어지는 객체 (앞으로 만들어질 객체)

    // 속성
    this.grade=grade; // 매개변수 grade 값을 생성되는 객체의 grade에 대입
    this.ban=ban;
    this.name=name;
...
document.getElementById("btn3").addEventListener("click", function(){
    ...
    // ** JSON 변환 시 객체의 기능은 포함하지 않는다 ** -> JSON은 데이터 전달에 특화된 형태

    const member = '{"memberId":"user01","memberPw":"pass01","age":20}';

    console.log(member); // JSON 문자열
    console.log(JSON.parse(member)); // JSON 문자열 -> 객체 변환
